// GomezGlobalFunctions.js  Version 009
...
var gomezDate = {
   init : function() {
      try {
         if (gomezNative) {
            ...
            if (gomezNative.FULLMONTHS) {
               this._FULLMONTHS = this.FULLMONTHS;
               this.FULLMONTHS = gomezNative.FULLMONTHS;
            }            
            if (gomezNative.DAYOFWEEK) {
               this._DAYOFWEEK = this.DAYOFWEEK;
               this.DAYOFWEEK = gomezNative.DAYOFWEEK;
            } 
            if (gomezNative.FULLDAYOFWEEK) {
               this._FULLDAYOFWEEK = this.FULLDAYOFWEEK;
               this.FULLDAYOFWEEK = gomezNative.FULLDAYOFWEEK;
            }                         
   ...
   //    m - abbreviated month (default language: English [Jan/Feb/Mar/...]
   //    mm - full month (default language: English [January/February/March/...]   
   //    w - abbreviated day of week (default language: English [Sun/Mon/Tue/...]
   //    ww - full day of week (default language: English [Sunday/Monday/Tuesday/...]   
   //    M - month
   //    MM - 2-digit month
   //    D - date
   //    DD - 2-digit date
      ...
      switch (format) {
         ...
         case 'CUSTOM':
            ...
            for (var i = 0; i < custom.length; i = i + 1) {
               ...
               switch (custom[i]) {
               	  case 'm':
               	     rv += gomezDate.MONTHS[date.getMonth()];
               	     break;
               	  case 'mm':
               	     rv += gomezDate.FULLMONTHS[date.getMonth()];
               	     break;
               	  case 'w':
               	     rv += gomezDate.DAYOFWEEK[date.getDay()];
               	     break;
               	  case 'ww':
               	     rv += gomezDate.FULLDAYOFWEEK[date.getDay()];
               	     break;
                  case 'M':
                     rv += month;
                     break;
   ...
   startsWithLCText : function(obj, val) {
      if (obj.text) {
         var objText = (obj.text.toLowerCase()).substring(0, val.length);
         return (objText == val.toLowerCase()) ? 1 : -1;
      }
      return -1;
   },
